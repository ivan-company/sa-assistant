...
class GoogleDriveAPI(GoogleAPI):
    ...
    def read_file(
        self, file_id: str, download_path: Optional[str] = None
    ) -> Union[bytes, str]:
        """
    ...
    def _export_google_file(
        self, file_id: str, mime_type: str, download_path: Optional[str] = None
    ) -> Union[bytes, str]:
    ...
    def list_files(
        ...
        page_size: int = 100,
        fields: str = "files(id, name, mimeType, parents, createdTime, modifiedTime, size, shortcutDetails)",
    ) -> List[Dict[str, Any]]:
        """
    ...
    def get_folder_id_by_path(
        ...
        for i, folder_name in enumerate(folders):
            ...
            if i == 0 and search_shared:
                ...
                query = f"name = '{folder_name}' and {mime_query} and trashed = false"
                if parent_id == "root":
                    # Include both root and shared folders
                    query = (
                        f"('{parent_id}' in parents or sharedWithMe = true) and {query}"
                    )
                else:
            ...
            if results:
                ...
                try:
                    ...
                    if error.resp.status == 403:
                        print(
                            f"No permission to create folder '{folder_name}' in parent"
                        )
                        return None
                    raise
            else:
    ...
    def download_file_by_path(
        self, file_path: str, local_download_path: Optional[str] = None
    ) -> Union[bytes, str]:
        """
